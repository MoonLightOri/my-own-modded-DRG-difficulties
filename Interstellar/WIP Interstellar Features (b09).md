As Interstellar is still a WIP difficulty, this feature list may be subject to change. This list is updated to b09 version.
...
- *new* Mactera Solaris (DEA, CD2)
  - Mactera Solaris is a fairly rare Mactera Radial Jaw enemy (a trijaw that shoots in a circular pattern), except it shoots Bet-C bomb that deals dread fireball tier damage, move very fast. They have 50% more HP than normal trijaw. Freeze temp is -200 celcius. There can only be 1 in the map. It's look is changed via CD2 (only the weakpoint is changed to red color).
- *new* The Sun (CD2)
