As Interstellar is still a WIP difficulty, this feature list may be subject to change. This list is updated to b28 version.
...
- (new) Added NitraMultiplier for Deep Dive Secondary with values: +25% on Eggs, +20% on Black Box and Dread Cocoon, +10% on Deep Scan and Morkite Well, unchanged in Mini-Mules and Morkite. This value is also compatible with current Main objective if they have nitra multiplier support, calculated additively.
...
- *new** Ice Spreader (CD2)
  - They are the promoted variant of Septic Spreader which lobs Snowball projectile at your team, maximum damage they can deal is 2/3 of Freezing bar (direct hit) and the damage will drop off on distance. Its look is modified via CD2.
- Grabber
...
- *new* Carrier (CD2 - inspiration from DH difficulty)
  - This is an elite version of promoted type of grabber that has x2 damage taken and has s5 stalker's cloak material. Instead they move extremely fast and only have 5s of Grab Time. It's banned in Defensive objective, Drillevator, additionally in Encounters and as well as in 1 player count lobby. Ignition and Frozen temp is the same as normal Grabber (30c and -150c). It's Chasing and Fleeing speed is reduced when downed player is detected in the map (scale with player count), maximum reduced value is 65% and they additionally uncloak their material when only 1 alive player is left.
- Bulk Detonator
...
- *new* Naedocyte Thugger (CD2)
  - These have a rare chance of spawning in a wave that has x2 damage taken (similar to normal breeder). These spawn swarmer sized Oppressor named "Glyphid Thug" with ~5% of original HP, deals 35% less damage than normally, halved freeze temperature and leave behind Glacial Praetorian's gas once died. Thugger and Thug's look is modified by CD2.
...
- *new* Wallhacker + Banshee (DEA)
  - These are the promoted version of twins. Both have Brundle HP. Wallhacker is a type of Lacerator that deals 25% less damage than normal lacerator, have different (intentionally bugged) AI behavior that prefers spamming stomp attack at your team in a far distance, even without line of sight. Banshee is a type of Arbalest that only uses Grieves attack. They also have different look via CD2.
...
- Big Shredder (CD2)
  - Possible with CD2, this is just a big Shredder that spawns alone or in a pair. It deals x2 normal Shredder damage, but it has about x13 the HP of a normal Shredder.
- Shredders
