As Interstellar is still a WIP difficulty, this feature list may be subject to change. This list is updated to b29 version.
...
- *new* Carrier (CD2 - inspiration from DH difficulty)
  - This is an elite version of promoted type of grabber that has x2 damage taken and has s5 stalker's cloak material. Instead they move extremely fast and only have 5s of Grab Time. It's banned in Defensive objective, Drillevator, additionally in Encounters and as well as in 1 player count lobby. Ignition and Frozen temp is the same as normal Grabber (30c and -150c). It's Chasing and Fleeing speed is reduced when downed player is detected in the map (scale with player count), maximum reduced value is 75% and they additionally uncloak their material when only 1 alive player is left.
- Bulk Detonator
